    ...
    <div className="min-h-screen bg-white text-slate-900">
      <div className="relative">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 py-10">
          ...
          {!application ? (
            /* ─── No Application Yet ─── */
            <div className="bg-white backdrop-blur-xl border border-slate-200 rounded-3xl p-10 shadow-2xl max-w-lg mx-auto">
              <h2 className="text-2xl font-bold text-slate-900 mb-2">Start Your Application</h2>
              ...
              <form onSubmit={handleAppSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <input type="text" required placeholder="e.g. Delhi Public School"
                    className="w-full bg-white border border-slate-300 text-slate-900 p-3.5 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 transition placeholder-slate-500"
                    value={formData.previous_school_name} onChange={e => setFormData({ ...formData, previous_school_name: e.target.value })}
                  />
                </div>
                <button type="submit" className="w-full bg-gradient-to-r from-blue-600 to-cyan-600 text-slate-900 font-bold py-3.5 rounded-xl shadow-lg hover:shadow-blue-900/30 transition-all hover:scale-[1.01] active:scale-[0.99]">
                  Submit Application
                </button>
              </form>
            </div>
            ...
            <>
              {/* Tab Bar */}
              <div className="flex space-x-1 bg-white backdrop-blur p-1.5 rounded-2xl mb-8 border border-slate-100 overflow-x-auto">
              ...
              <div className="bg-white backdrop-blur-xl border border-slate-200 rounded-3xl p-8 shadow-2xl">
                ...
                {activeTab === 'status' && (
                  <div>
                    <h2 className="text-2xl font-bold text-slate-900 mb-6">Application Overview</h2>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                      ...
                      <div className="bg-slate-50/50 rounded-2xl p-5 border border-slate-200">
                        <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Entry Type</p>
                        <p className="text-lg font-bold text-slate-900">{application.entry_type}</p>
                      </div>
                      <div className="bg-slate-50/50 rounded-2xl p-5 border border-slate-200">
                        <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Enrollment</p>
                        <p className="text-lg font-bold text-cyan-400">{enrollment ? enrollment.enrollment_number : 'Not Yet'}</p>
                      </div>
                      <div className="bg-slate-50/50 rounded-2xl p-5 border border-slate-200">
                        <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">CGPA</p>
                ...
                {activeTab === 'docs' && (
                  <div>
                    ...
                    <form onSubmit={handleDocUpload} className="bg-slate-50/50 rounded-2xl p-6 border border-slate-200 mb-8 space-y-4">
                      <div>
                        ...
                        <input type="text" placeholder="e.g. Aadhaar Card, 10th Marksheet" required
                          className="w-full bg-white border border-slate-300 text-slate-900 p-3 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 placeholder-slate-500"
                      ...
                      <button type="submit" className="bg-blue-600 hover:bg-blue-500 text-slate-900 px-6 py-2.5 rounded-xl font-semibold transition">Upload Document</button>
                    </form>
                    <h3 className="text-lg font-semibold text-slate-700 mb-4">Submitted Documents</h3>
                    {application.documents?.length > 0 ? (
                      <div className="space-y-3">
                        {application.documents.map((d: any) => (
                          <div key={d.id} className="bg-slate-50/50 p-4 border border-slate-200 rounded-xl flex items-center justify-between">
                ...
                {activeTab === 'scholarship' && (
                  <div>
                    <h2 className="text-2xl font-bold text-slate-900 mb-6">Merit Scholarship</h2>
                    {application.scholarship ? (
                      ...
                      <form onSubmit={handleScholarship} className="space-y-4">
                        ...
                        <textarea required placeholder="Describe why you deserve this scholarship…"
                          className="w-full bg-white border border-slate-300 text-slate-900 p-4 rounded-xl h-32 outline-none focus:ring-2 focus:ring-blue-500 placeholder-slate-500 resize-none"
                          value={scholarshipReason} onChange={e => setScholarshipReason(e.target.value)} />
                        <button type="submit" className="bg-blue-600 hover:bg-blue-500 text-slate-900 px-6 py-2.5 rounded-xl font-semibold transition">Apply for Scholarship</button>
                      </form>
                    )}
                  </div>
                )}
                ...
                {activeTab === 'enrollment' && (
                  <div>
                    <h2 className="text-2xl font-bold text-slate-900 mb-6">Enrollment & Fee Payment</h2>
                    {enrollment ? (
                      <div className="text-center py-10">
                        ...
                        <p className="text-4xl font-mono font-black text-slate-900 tracking-widest bg-slate-50/50 inline-block px-8 py-4 rounded-2xl border border-slate-200">{enrollment.enrollment_number}</p>
                      </div>
                    ) : (
                      <div className="text-center py-10">
                        <p className="text-slate-400 mb-2">All documents must be verified before you can pay.</p>
                        <p className="text-slate-400 text-sm mb-8">Pay the seat fee to confirm your admission and get your enrollment number.</p>
                        <button onClick={payFee} className="bg-gradient-to-r from-blue-600 to-cyan-600 text-slate-900 px-10 py-4 rounded-2xl font-bold text-lg shadow-lg hover:shadow-blue-900/30 transition-all hover:scale-[1.02] active:scale-[0.98]">
                ...
                {activeTab === 'academics' && (
                  <div>
                    <h2 className="text-2xl font-bold text-slate-900 mb-6">Academics & Attendance</h2>
                    {myRegistrations.length > 0 ? (
                      <div className="space-y-8">
                        {/* Live Attendance */}
                        <div>
                          <h3 className="text-lg font-semibold text-slate-700 border-b border-slate-200 pb-3 mb-4 flex items-center">
                          ...
                          {attendance.length > 0 ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              {attendance.map((att: any, idx: number) => (
                                <div key={idx} className="bg-slate-50/50 border border-slate-200 rounded-2xl p-5">
                                  <div className="flex justify-between items-end mb-3">
                                    <div>
                                      <p className="font-bold text-slate-900">{att.course_code}</p>
                                      <p className="text-xs text-slate-400">{att.course_name}</p>
                                    </div>
                                  ...
                                  <div className="flex justify-between mt-2">
                                    <p className="text-xs text-slate-400">{att.present} / {att.total} classes</p>
                                    {att.warning && <span className="text-[10px] font-bold text-red-400 uppercase">⚠ Low Attendance</span>}
                                  </div>
                                </div>
                              ))}
                            </div>
                          ) : <p className="text-slate-400 italic">No attendance recorded yet.</p>}
                        </div>
                        {/* Registered Courses */}
                        <div>
                          <h3 className="text-lg font-semibold text-slate-700 border-b border-slate-200 pb-3 mb-4">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                            {myRegistrations[0]?.courses_details?.map((c: any) => (
                              <div key={c.id} className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                                <span className="font-bold text-blue-700 block">{c.code}</span>
                                <span className="text-sm text-blue-600">{c.name}</span>
                        ...
                        <div>
                          ...
                          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
                            <div className="space-y-4">
                              {myRegistrations[0]?.courses_details?.map((c: any) => (
                                <div key={c.id} className="flex items-center justify-between p-3 hover:bg-slate-50 rounded-xl border border-transparent hover:border-slate-100 transition">
                                  <div>
                                    <p className="font-bold text-slate-900">{c.code} - Syllabus & Notes</p>
                                    <p className="text-xs text-slate-500">Updated recently</p>
                                  </div>
                                  <button className="text-sm bg-purple-50 hover:bg-purple-100 text-purple-600 px-4 py-1.5 rounded-lg font-medium border border-purple-200 transition">
                                    View Materials
                                  </button>
                                </div>
                              ))}
                      ...
                      <div>
                        <h3 className="text-lg font-semibold text-slate-700 mb-2">Register for Courses</h3>
                        <p className="text-sm text-slate-400 mb-6">Select the courses you want to take this semester.</p>
                ...
                {activeTab === 'timetable' && (
                  <div>
                    <h2 className="text-2xl font-bold text-slate-900 mb-6">Weekly Timetable</h2>
                    {timetable.length > 0 ? (
                      <div className="space-y-4">
                          ...
                          <div key={day}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                              {timetable.filter(t => t.day === day).map((slot: any) => (
                                <div key={slot.id} className="bg-slate-50/50 border border-slate-200 rounded-xl p-4">
                                  <p className="font-bold text-slate-900">{slot.course_code}</p>
                                  ...
                                  <div className="flex justify-between items-center mt-3 pt-3 border-t border-slate-200">
                                    ...
                                    {slot.room && <span className="text-xs bg-white px-2 py-1 rounded text-slate-400">🏛 {slot.room}</span>}
                                  </div>
                                  {slot.faculty_name && <p className="text-xs text-slate-400 mt-1">👤 {slot.faculty_name}</p>}
                                </div>
                              ))}
                            </div>
                          </div>
                      ...
                      <div className="text-center py-10 bg-slate-50/30 rounded-2xl border border-slate-200/50">
                ...
                {activeTab === 'fees' && (
                  <div>
                    <h2 className="text-2xl font-bold text-slate-900 mb-6">Fee Management</h2>
                    {fees.length > 0 ? (
                      <div className="space-y-3">
                        {fees.map((fee: any) => (
                          <div key={fee.id} className="bg-slate-50/50 border border-slate-200 rounded-2xl p-5">
                            <div className="flex items-center justify-between">
                              <div>
                                <p className="font-bold text-slate-900 text-lg">Semester {fee.semester}</p>
                                ...
                                <p className="text-xs text-slate-400 mt-1">Due: {fee.due_date}</p>
                              </div>
                              <div className="flex items-center space-x-3">
                                ...
                                {fee.status !== 'PAID' && (
                                  <button onClick={() => paySemesterFee(fee.id)}
                                    className="bg-gradient-to-r from-blue-600 to-cyan-600 text-slate-900 px-4 py-2 rounded-lg font-semibold text-sm transition hover:scale-[1.02]">
                                    Pay Now
                                  </button>
                                )}
                              </div>
                ...
                {activeTab === 'lifecycle' && (
                  <div>
                    <h2 className="text-2xl font-bold text-slate-900 mb-6">Leave of Absence (LOA)</h2>
                    <form onSubmit={handleLeaveSubmit} className="bg-slate-50/50 rounded-2xl p-6 border border-slate-200 mb-8 space-y-4">
                      ...
                      <button type="submit" className="bg-blue-600 hover:bg-blue-500 text-slate-900 px-6 py-2.5 rounded-xl font-semibold transition">Request Leave</button>
                    </form>
                    <h3 className="text-lg font-semibold text-slate-700 mb-4">Past Requests</h3>
                    {leaves.length > 0 ? (
                      <div className="space-y-3">
                        {leaves.map((l: any) => (
                          <div key={l.id} className="bg-slate-50/50 p-4 border border-slate-200 rounded-xl flex items-center justify-between">
                            <div>
                              <span className="font-medium text-slate-900">{l.start_date} to {l.end_date}</span>
                ...
                {activeTab === 'results' && (
                  <div>
                    <div className="flex items-center justify-between mb-6">
                      <h2 className="text-2xl font-bold text-slate-900">Examination Results</h2>
                      {cgpa > 0 && (
                        <div className="flex items-center space-x-4">
                          {Object.entries(sgpa).map(([sem, gpa]) => (
                            <div key={sem} className="bg-slate-50/50 px-4 py-2 rounded-xl border border-slate-200 text-center">
                              <p className="text-[10px] text-slate-400 uppercase">Sem {sem}</p>
                    ...
                    {results.length > 0 ? (
                      <>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                          {results.map((r: any) => (
                            <div key={r.id} className="bg-slate-50/50 border border-slate-200 rounded-2xl p-5 relative overflow-hidden">
                              <div className="absolute right-[-10%] bottom-[-20%] text-9xl font-black opacity-5 text-slate-900 pointer-events-none">{r.grade}</div>
                              <div className="flex justify-between items-start mb-4 relative z-10">
                                <div>
                                  <p className="font-bold text-slate-900 text-lg">{r.course_code}</p>
                                ...
                                <div className="flex flex-col items-end">
                                  ...
                                  <span className="text-xs text-slate-400 font-medium tracking-widest uppercase">Grade</span>
                                </div>
                              </div>
                              <div className="flex justify-between items-center pt-4 border-t border-slate-200 relative z-10">
                                <div>
                                  <span className="text-2xl font-bold text-slate-900">{r.marks_obtained}</span>
                                  <span className="text-sm text-slate-400"> / {r.max_marks}</span>
                                </div>
                                <div className="flex gap-2">
                                  ...
                                  {!r.is_backlog && !r.is_revaluation && <span className="px-2 py-1 bg-slate-100 text-slate-700 text-slate-400 rounded text-[10px] font-bold uppercase">Regular</span>}
                                </div>
                              </div>
                            </div>
                          ))}
                        ...
                        <div className="border-t border-slate-200 pt-6">
                          ...
                          <form onSubmit={submitRevaluation} className="bg-slate-50/50 rounded-2xl p-6 border border-slate-200 space-y-4">
                            <div>
                              <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Select Result</label>
                              <select className="w-full bg-white border border-slate-300 text-slate-900 p-3 rounded-xl outline-none focus:ring-2 focus:ring-blue-500"
                            ...
                            <div>
                              ...
                              <textarea placeholder="Explain why you want revaluation..." required
                                className="w-full bg-white border border-slate-300 text-slate-900 p-3 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 h-20 resize-none"
                                value={revalForm.reason} onChange={e => setRevalForm({ ...revalForm, reason: e.target.value })} />
                            </div>
                            <button type="submit" className="bg-blue-600 hover:bg-blue-500 text-slate-900 px-6 py-2.5 rounded-xl font-semibold transition">Submit Revaluation</button>
                          </form>
                          {revaluations.length > 0 && (
                            <div className="mt-6 space-y-3">
                              <h4 className="text-sm font-bold text-slate-400 uppercase">Your Revaluation Requests</h4>
                              {revaluations.map((rv: any) => (
                                <div key={rv.id} className="bg-slate-50/50 p-4 border border-slate-200 rounded-xl flex items-center justify-between">
                                  <div>
                                    <p className="font-medium text-slate-900">{rv.course_code} — Original: {rv.original_grade} ({rv.original_marks})</p>
                                    <p className="text-xs text-slate-400">{rv.reason}</p>
                                  </div>
                                  <span className={`text-xs font-bold px-3 py-1 rounded-full border ${
                ...
                {activeTab === 'transfer' && (
                  <div>
                    <h2 className="text-2xl font-bold text-slate-900 mb-6">Transfer / Exit Documentation</h2>

                    {/* No-Dues Status */}
                    {noDues && (
                      <div className="bg-slate-50/50 rounded-2xl p-6 border border-slate-200 mb-8">
                        <h3 className="text-lg font-semibold text-slate-700 mb-4">No-Dues Clearance Status</h3>
                    ...
                    <form onSubmit={submitTransfer} className="bg-slate-50/50 rounded-2xl p-6 border border-slate-200 mb-8 space-y-4">
                      <h3 className="text-lg font-semibold text-slate-700">Submit a Request</h3>
                      <div>
                      ...
                      <button type="submit" className="bg-red-600 hover:bg-red-500 text-slate-900 px-6 py-2.5 rounded-xl font-semibold transition">Submit Request</button>
                    </form>
                    ...
                    {transferRequests.length > 0 && (
                      <div className="space-y-3">
                        ...
                        {transferRequests.map((tr: any) => (
                          <div key={tr.id} className="bg-slate-50/50 p-4 border border-slate-200 rounded-xl flex items-center justify-between">
                            <div>
                              <span className="font-medium text-slate-900">{tr.request_type.replace('_', ' ')}</span>
                              <p className="text-xs text-slate-400 mt-1">{tr.reason}</p>
                            </div>
                            <span className={`text-xs font-bold px-3 py-1 rounded-full border ${
